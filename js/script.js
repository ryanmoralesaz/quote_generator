var refreshPrint = setInterval(
  function() {
    printQuote();
  }, // Set the initial interval to call the printQuote function without clicking loadquote button
  15000
); // set the interval to 10 seconds
document
  .getElementById("loadQuote")
  .addEventListener("click", printQuote, false); // add event listener to respond to "Show another quote" button clicks // when user clicks anywhere on the button, the "printQuote" function is called
var quotes = [
  // declare the quotes array at the global level
  //Create the array of object literals that contain the key : value pairs for quote, source, citation and year
  {
    ...
    quote:
      "The artist is nothing without the gift, but the gift is nothing without work.", //quote string
...
function countQuoteLines() {
  // Create a function to check how tall a quote is. If it's too tall then reduce font size.
  var quoteHeight = document.getElementById("quote-box").firstChild
    .offsetHeight; // set the quote height to the height of quote paragraph, the first child of the quote-box div
  // check if the quote height is greater than a relative amount to 100% screen size to avoid ovelap with button
  if (quoteHeight >= 210) {
    document.getElementById("quote-box").firstChild.style.fontSize = "3rem";
  }
  if (quoteHeight >= 260) {
    document.getElementById("quote-box").firstChild.style.fontSize = "2.8rem";
  }
}
function getRandomQuote() {
  // declare the getRandomQuote function
...
function randomBackgroundColor() {
  //declare function to set random background color to rgb values between 0 "black" and 255 "white"
  ...
  if ((r > 150 && g > 150) || (r > 150 && b > 150) || (b > 150 && g > 150)) {
    document.body.style.color = "black";
  } else {
    document.body.style.color = "white";
  } // change the color of the text depending how light or dark the background color
}
...
function printQuote() {
  // declare the printQuote function
  ...
  if (!selectedQuotes) {
    //If selected quotes is false, this will run the first time printQuote() is called
    ...
    // if selectedQuotes has at least one object
    ...
    do {
      ...
      if (selectedQuotes.length == quotes.length) {
      ...
      } else if (fullQuote == selectedQuotes[count]) {
        // if not all quotes have been used, check the new quote against array
        ...
        count++;
      } // if matching quote has not been found, continue to test the loop
  ...
  quoteText =
    ...
    fullQuote.source +
    (fullQuote.citation
      ? '<span class="citation">' + fullQuote.citation + "</span>"
      : "") +
    (fullQuote.year ? '<span class="year">' + fullQuote.year + "</span>" : "") +
    (fullQuote.tags
      ? '<span class="citation"> Tags:' + fullQuote.tags + "</span>"
      : "") +
    "</p>"; //create the html string that will replace the innerHTML, concatenate using dot nation on the quote object
  ...
  return (refreshPrint = setInterval(function() {
    printQuote();
  }, 15000)); // reset the refresh printQuote interval to change after 15 seconds
}
